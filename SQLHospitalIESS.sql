...
GO
CREATE TABLE tblHistoriaClinicaDoctor (numeroHistoria int 
FOREIGN KEY REFERENCES tblHistoriaClinica (numeroHistoria) ON UPDATE CASCADE ON DELETE CASCADE, fecha datetime,
temperaturaPaciente float, altura float, peso float, diagnostico varchar(100), indicaciones varchar(100), cedulaDoctor varchar(10) 
FOREIGN KEY REFERENCES tblDoctor (cedulaDoctor) ON UPDATE CASCADE)
GO
...
FOREIGN KEY REFERENCES tblEspecialidad (codigoEspecialidad) ON UPDATE CASCADE, cedulaRecepcionista varchar(10) 
FOREIGN KEY REFERENCES tblRecepcionista (cedulaRecepcionista) ON UPDATE CASCADE, cedulaDoctor varchar(10) 
FOREIGN KEY REFERENCES tblDoctor (cedulaDoctor), estado varchar(9))
GO
...
ON UPDATE CASCADE, cedulaDoctor varchar(10) FOREIGN KEY REFERENCES tblDoctor (cedulaDoctor) ON UPDATE CASCADE, 
...
tblMedicamento (codigoMedicamento) ON UPDATE CASCADE, cantidad int, subtotal money)
GO
CREATE TABLE tblReceta (idReceta int PRIMARY KEY IDENTITY, cedulaPaciente varchar(10) FOREIGN KEY REFERENCES 
tblPaciente (cedulaPaciente) ON UPDATE CASCADE ON DELETE CASCADE, cedulaDoctor varchar(10) FOREIGN KEY REFERENCES 
...
GO
INSERT INTO tblAdministrador VALUES ('1716116809', 'Alejandro Esteban', 'Guerrero', 'Tipán', '21/08/1994', 'Masculino', 'alejandro.guerrero@epn.edu.ec', 
'Pichincha', 'Quito', 'El Inca', '0987858621', 'alejandroG', 'holaMundo')
